...
export function ProjectDetailModal({ project, content, onClose }) {
  ...
  const lists = ["features", "technologies", "metrics"].filter(
    (field) => project[field]?.length,
  );
  const gallery =
    project.gallery?.filter((image) => image.src && image.alt) || [];
  const collaborators =
    project.collaborators?.filter((person) => person.name) || [];
      ...
      <div className="project-detail-content">
        ...
        {lists.map((field) => (
          <section className="project-detail-section" key={field}>
            <h3>
              {field === "features"
                ? project.featuresLabel || labels.features
                : labels[field]}
            </h3>
            <ul
              className={
